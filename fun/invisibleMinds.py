...
def videoFightModel(tf,wight='invisibleMinds.hdfs'):
...
def videoFightModel2(tf,wight='invisibleMinds.hdfs'):
    layers = tf.keras.layers
    models = tf.keras.models
    #losses = tf.keras.losses
    optimizers = tf.keras.optimizers
    ...
    np.random.seed(1234)
    vg19 = tf.keras.applications.vgg19.VGG19
    base_model = vg19(include_top=False,weights='imagenet',input_shape=input_shapes)
